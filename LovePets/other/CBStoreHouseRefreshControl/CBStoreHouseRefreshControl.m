...
- (BOOL)isRefreshing{
    return self.state == CBStoreHouseRefreshControlStateRefreshing;
}
...
- (void)scrollViewDidEndDragging
{
    if ((self.state == CBStoreHouseRefreshControlStateIdle && self.realContentOffsetY < -self.dropHeight)) {
        ...
        if (self.state == CBStoreHouseRefreshControlStateRefreshing) {
            ...
            self.originalTopContentInset = 1;
...
- (void)finishingLoading
{
    ...
    if (self.state == CBStoreHouseRefreshControlStateDisappearing) {
        return;
    }
